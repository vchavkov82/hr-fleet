...
const MODULE_KEYS = ['coreHr', 'ats', 'leave', 'payroll', 'performance', 'onboarding'] as const
...
const FEATURE_MATRIX: Record<string, boolean[]> = {
  employeeProfiles:   [true,  false, false, false, false, false],
  orgChart:           [true,  false, false, false, false, false],
  documentStorage:    [true,  false, false, false, false, false],
  jobPosting:         [false, true,  false, false, false, false],
  candidatePipeline:  [false, true,  false, false, false, false],
  aiScreening:        [false, true,  false, false, false, false],
  leaveRequests:      [false, false, true,  false, false, false],
  leaveCalendar:      [false, false, true,  false, false, false],
  accrualPolicies:    [false, false, true,  false, false, false],
  payrollProcessing:  [false, false, false, true,  false, false],
  taxReporting:       [false, false, false, true,  false, false],
  bankIntegration:    [false, false, false, true,  false, false],
  goalTracking:       [false, false, false, false, true,  false],
  reviews360:         [false, false, false, false, true,  false],
  performanceReports: [false, false, false, false, true,  false],
  taskChecklists:     [false, false, false, false, false, true ],
  welcomePortal:      [false, false, false, false, false, true ],
  eSignatures:        [false, false, false, false, false, true ],
}
...
export default async function PricingPage({
  ...
  const moduleNames = Object.fromEntries(
    MODULE_KEYS.map((k) => [k, t(`modules.${k}.name`)])
  )
            ...
            <table className="w-full min-w-[700px] border-collapse">
              <thead>
                <tr className="bg-navy-deep text-white">
                  <th className="text-left py-4 px-4 text-sm font-semibold rounded-tl-xl">
                    {t('comparisonFeatureCol')}
                  </th>
                  {MODULE_KEYS.map((key, i) => (
                    <th
                      key={key}
                      className={`text-center py-4 px-3 text-sm font-semibold ${i === MODULE_KEYS.length - 1 ? 'rounded-tr-xl' : ''}`}
                    >
                      {t(`modules.${key}.name`)}
                    </th>
                  ))}
              ...
              <tbody>
                {FEATURE_ROWS.map((row, idx) => (
                  <tr
                    key={row}
                    className={idx % 2 === 0 ? 'bg-white' : 'bg-surface-lighter'}
                  >
                    <td className="py-3 px-4 text-sm text-gray-700">
                      {t(`comparison.${row}`)}
                    </td>
                    {FEATURE_MATRIX[row].map((included, colIdx) => (
      ...
      <section className="py-20 bg-surface-lighter">
        <div className="container-xl max-w-3xl">
          ...
          <div className="space-y-4">
            {(t.raw('faqItems') as Array<{ question: string; answer: string }>).map((item, idx) => (
              <details
                key={idx}
