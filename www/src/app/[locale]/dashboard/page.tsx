...
export default async function DashboardPage({
  ...
  // Fetch real employee count from API (graceful fallback to --)
  let employeeCount: number | string = '--'
  try {
    const cookieStore = await cookies()
    const token = cookieStore.get('auth_token')?.value
    if (token) {
      const res = await fetchEmployees(token, { perPage: 1 })
      employeeCount = res.pagination.total
    }
  } catch {
    // Graceful fallback -- metric card shows '--' if API unavailable
  }
