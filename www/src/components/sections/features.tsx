        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {FEATURE_KEYS.map((key) => (
            <Link
              key={key}
              href={`/features#${key}`}
              className="bg-white rounded-xl p-8 border border-gray-200 hover:shadow-lg hover:border-primary-200 transition-all group block cursor-pointer"
            >
              <div className="w-12 h-12 bg-primary-50 rounded-lg flex items-center justify-center mb-4">
                {ICONS[key]}
              </div>
              <h3 className="text-xl font-semibold font-heading text-navy mb-2 group-hover:text-primary transition-colors">
                {t(`items.${key}.title`)}
              </h3>
