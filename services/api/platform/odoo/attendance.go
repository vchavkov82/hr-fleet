...
func (c *Client) ListAttendance(domain []any, limit, offset int) ([]AttendanceRecord, int, error) {
	if domain == nil {
		domain = []any{}
	}

	count, err := c.SearchCount("hr.attendance", domain)
	if err != nil {
		return nil, 0, fmt.Errorf("list attendance count: %w", err)
	}

	records, err := c.SearchRead("hr.attendance", domain, attendanceFields, limit, offset)
	if err != nil {
...
func (c *Client) CheckIn(employeeID int64) (int64, error) {
	vals := map[string]any{"employee_id": employeeID}
	id, err := c.Create("hr.attendance", vals)
	if err != nil {
...
func (c *Client) CheckOut(attendanceID int64) error {
	// Odoo auto-fills check_out on write when the field is set
	return c.Write("hr.attendance", attendanceID, map[string]any{
		"check_out": "now",
	})
}
